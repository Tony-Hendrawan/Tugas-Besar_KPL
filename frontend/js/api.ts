...
async function apiFetch(url, options) {
  ...
  try {
    ...
    if (!contentType || !contentType.includes('application/json')) {
      return { success: false, data: null, message: 'Server returned non-JSON response (' + res.status + ')' };
    }

    var json = await res.json();
    if (!res.ok) return { success: false, data: null, message: json.message || ('HTTP ' + res.status) };
    return json;
  } catch (error) {
