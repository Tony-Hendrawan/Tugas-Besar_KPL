...
if (wishlistEl) {
  ...
  async function loadWishlist() {
    ...
    if (list.length === 0) {
      wishlistEl.innerHTML =
        '<div style="grid-column:1/-1;text-align:center;padding:3rem;"><p style="color:var(--gray-500);margin-bottom:1rem;">Belum ada kos di wishlist.</p><a href="kos-list.html" class="btn-primary">Cari Kos</a></div>';
      return;
    }

    wishlistEl.innerHTML = list
      .map(function (kos) {
        var img = getKosImagePath(kos);
        var price = kos.harga_min || 0;
        return (
          '<div class="kos-card">' +
          '<div class="kos-image"><img src="' +
          img +
          '" alt="' +
          ...
          '<a href="kos-detail.html?id=' +
          kos.kos_id +
          '" class="btn-detail">Lihat Detail</a>' +
          '<button class="btn-outline" style="width:100%;color:var(--danger);border-color:var(--danger);" data-remove-wishlist="' +
          kos.kos_id +
    ...
    wishlistEl
      .querySelectorAll("[data-remove-wishlist]")
      .forEach(function (btn) {
        btn.addEventListener("click", async function () {
          var kosId = this.dataset.removeWishlist;
          var res = await apiFetch("/api/wishlist/" + kosId, {
            method: "DELETE",
          });
