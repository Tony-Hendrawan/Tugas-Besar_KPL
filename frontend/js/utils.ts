...
function getKosImagePath(kos) {
  ...
  if (
    kos &&
    kos.kamar_kos &&
    kos.kamar_kos.length > 0 &&
    kos.kamar_kos[0].foto
  ) {
  ...
  if (
    /^data:image\/(?:jpeg|jpg|png|webp);base64,[A-Za-z0-9+/=]+$/.test(imageUrl)
  )
    return imageUrl;
  if (imageUrl.startsWith("images/")) return imageUrl;
  if (/^https?:\/\/.+/i.test(imageUrl)) return imageUrl;
  return getDefaultKosImage(
    kos && (kos.kos_id || kos.id) ? kos.kos_id || kos.id : 1,
  );
}
...
function addToCompare(kos) {
  ...
  if (
    ...
    return { ok: false, msg: "Kos sudah ada di daftar bandingkan." };
  if (list.length >= MAX_COMPARE)
    return { ok: false, msg: "Maksimal " + MAX_COMPARE + " kos." };
  list.push(kos);
...
function removeFromCompare(kosId) {
  saveCompareList(
    getCompareList().filter(function (k) {
      return k.id !== kosId;
    }),
  );
}
